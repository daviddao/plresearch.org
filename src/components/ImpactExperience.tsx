// Shared body of the Impact experience (hero → field-velocity dashboard →
// methodology). Rendered by both the canonical /impact route and the hidden,
// un-indexed preview route, so the two never drift. All data fetching lives
// here; the route files stay thin (metadata + gating only).
...
/**
 * The full impact experience. `preview` shows a small ribbon so anyone with the
 * secret link knows they are looking at an unlisted draft.
 */
export default async function ImpactExperience({ preview = false }: { preview?: boolean } = {}) {
    ...
    <div>
      {preview && (
        <div className="border-b border-amber-300/60 bg-amber-50 text-amber-900">
          <div className="max-w-6xl mx-auto px-6 py-2 text-xs sm:text-[13px] leading-relaxed">
            <strong className="font-semibold">Unlisted preview.</strong> A private draft shared by link —
            not linked from the site, not indexed, and still under review. Please don&rsquo;t share the URL.
          </div>
        </div>
      )}
